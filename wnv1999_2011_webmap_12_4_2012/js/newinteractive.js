...
var layername='EastWeb:RSWNV_MerCentroid';
//For data beyond 2010.
//var layername11='EastWeb:RSWNV11';
var layername11='EastWeb:RSWNV_MerCentroid';
...
map.addLayers([gmap,gphy,gsat,ghyb,wnvfx1999,wnvfx2000,wnvfx2001,wnvfx2002,wnvfx2003,wnvfx2004,wnvfx2005,wnvfx2006,wnvfx2007,wnvfx2008,wnvfx2009,wnvfx2010,wnvfx2011,wnvqn1999,wnvqn2000,wnvqn2001,wnvqn2002,wnvqn2003,wnvqn2004,wnvqn2005,wnvqn2006,wnvqn2007,wnvqn2008,wnvqn2009,wnvqn2010,wnvqn2011,state]);
...
this.map.events.register('click', map, function (e) {
        ...
        var maplayer=map.layers[29].params.LAYERS;         
       /* var params = {
...
function changeLayer(opacity,layer)
 {	
 	...
 	if(classtype=="manual")
 	{
 		DisplayLegend(layername,'ylorrd_mnrw_99'); // All mannual Incidene rates have same legend so no need to put redundent legend class in each images.
 		switch(value)
 		{
 			case 1999:
 			var n=4;
 			//alert(OpenLayers.getScale());
 			this.map.layers[4].setVisibility(true);
			this.map.layers[n].setOpacity(opacity);
 			 for(var i=n+1;i<=29;i++)
 			{
 	...
 	else
 	{
 		switch(value)
 		{
 			case 1999:
 			var n=17;
 			this.map.layers[17].setVisibility(true);
 			this.map.layers[n].setOpacity(opacity);
...
$(document).ready(function() { // It is equivalent to $(function(){});
	...
	$( "#mapslider" ).slider({
		...
		$( "#yearlabel" ).val( $( "#mapslider" ).slider( "value" ) ); // To dispaly incidence year
	...
	$.fn.extend({
		mapsliderLabels:function()
		{
	        ...
	        $('#mapslider').prepend('<span class="mapLabel" style="left:-7px;">1999</span>')   //Append labels to map slider bar
	       	.append('<span class="mapLabel" style="left:-6px;">2000</span>')
	        .append('<span class="mapLabel" style="left:0.2px;">2001</span>')
	        .append('<span class="mapLabel" style="left:4px;">2002</span>')
	        .append('<span class="mapLabel" style="left:8px;">2003</span>')
	        .append('<span class="mapLabel" style="left:11px;">2004</span>')
	        .append('<span class="mapLabel" style="left:15px;">2005</span>') 
	        .append('<span class="mapLabel" style="left:19px;">2006</span>')
	        .append('<span class="mapLabel" style="left:24px;">2007</span>')
	        .append('<span class="mapLabel" style="left:29px;">2008</span>')
	        .append('<span class="mapLabel" style="left:35px;">2009</span>')
	        .append('<span class="mapLabel" style="left:44px;">2010</span>')  		
	        .append('<span class="mapLabel" style="left:49px;">2011</span>');
	...
	$('.tooltip').tooltip({width: '300px', height: '50px'});							//Classification type tooltip
...
{	
	...
	var state=extract[2].slice(0,3);		//Extract state
	var county= extract[3].slice(0,-5);		//Extract county	
	var pop=extract[6].slice(0,-7);			//Extract Population
	var wnv99=extract[7].slice(0,-7);		//Extract incidence rates
	...
	var wnv11=extract[44].slice(0,-4);
	var llon=extract[45].slice(0,-4); 		//Extract lon value
	var llat=extract[46].slice(0,-4); 		//Extract lat value
	llat=llat.split('-');
	llat=llat[0];
	...
	*/
	...
	var chartOptions={width: 336, height: 168,colors:['FF0000'],title:county+','+state+' Population: '+pop,vAxis:{title: 'WNV cases',maxValue:8},hAxis:{title:"Years"},legend:"none",pointSize:2};
...
function DisplayLegend(layername,legendstyle)
{
	...
	oldimgId=imgId;
	lg_img.src=layerhost+"?REQUEST=GetLegendGraphic&VERSION=1.0.0&FORMAT=image/png&WIDTH=35&HEIGHT=22&LAYER="+layername+"&LEGEND_OPTIONS=forceRule:True;dx:0.2;dy:0.2;mx:0.2;my:0.2;fontStyle:bold;borderColor:0000ff;border:true;fontColor:000000;bgColor:a5a5a5;fontSize:14&STYLE="+legendstyle;
